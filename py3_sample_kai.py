...
def make_cost_map(m, miny):
    ...
    for y in range(maxy - 1, max(0, miny) - 1, -1):
        for x in range(m.w):
            if m.getXY(x, y) == 0:
                cost_m.setXY(x, y, cost_m.getXY(x, y + 1) + 15)
        for x in range(m.w):
            if m.getXY(x, y) == 0:
                v = min(cost_m.getXY(x, y), cost_m.getXY(x - 1, y) + 12)
                cost_m.setXY(x, y, v)
        ...
        for x in range(m.w):
            if m.getXY(x, y) == 0:
                if m.getXY(x, y + 1) == 0:
                    cost_m.setXY(x, y, cost_m.getXY(x, y) - 2)
                if m.getXY(x + 1, y) == 0 and m.getXY(x, y + 1) == 0:
                    cost_m.setXY(x, y, cost_m.getXY(x, y) - 2)
                if m.getXY(x - 1, y) == 0 and m.getXY(x, y + 1) == 0:
                    cost_m.setXY(x, y, cost_m.getXY(x, y) - 2)
    for y in range(maxy + 1, maxy + 4):
        for x in range(m.w):
            cost_m.setXY(x, y, cost_m.getXY(x, y - 1))
    return cost_m
...
def evaluate(ps, m, cost_m):
    ...
    return cost - 10 * np.sum(np.absolute(ps[1]))
    ...
    while True:
        ...
        for ax, ay in itertools.product(range(-1, 2), range(-1, 2)):
            ...
            if (next_pos[0] == ps[0][0]).all():
                continue
            v = search(next_pos, m, cost_m, depth)
            if v > maxv:
                maxv = v
                acs = (ax, ay)
        if maxv < -1000000.0:
